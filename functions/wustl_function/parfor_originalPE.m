...
if exist(this_originalpe_matname, 'file') ~= 2
    %load features
    ...
    if isempty(gv_info.matches)
        tent_xq2d = zeros(2, 0);
        tent_xdb2d = zeros(2, 0);
        tent_ray2d = zeros(3, 0);
        tent_xdb3d = zeros(3, 0);
        
        tentatives_2d = zeros(4, 0);
        tentatives_3d = zeros(6, 0);
    else
        tentative_qindex = gv_info.matches(1, :); tentative_dbindex = gv_info.matches(2, :);
    ...
    %save
    if exist(fullfile(params.output.pnp_sparse_origin.dir, qname), 'dir') ~= 7
        mkdir(fullfile(params.output.pnp_sparse_origin.dir, qname));
    end
    save('-v6', this_originalpe_matname, 'P', 'inls', 'tentatives_2d', 'tentatives_3d');
